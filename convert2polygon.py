...
def seg2poly(dataset_path='', plot=True):
    """
    ...
    for json_file in f:
        if json_file.find("instances_train2017.json") == -1 and json_file.find("instances_val2017.json") == -1 :
        # if json_file.find("instances_minitrain2017.json") == -1 and json_file.find("instances_val2017.json") == -1 :
            continue
        ...
        if not os.path.exists(str(anno_dir)): os.makedirs(str(anno_dir), exist_ok=True)
        ...
        for img_i, img in enumerate(tqdm(coco_data.dataset['images'])):
            ...
            plot = plot and plot_now<10 # test the first 10 images that are not crowded
            ...
            if img_name.exists():
                ...
                for object0 in coco_data.imgToAnns[img['id']]:
                    ...
                    if not "iscrowd" in object0 or not object0['iscrowd']:
                        segments = []
...
def polygon_plot_image(img_name, polygon_coords, segment_coords=None):
    ...
    plt.axis('off')
    # plt.show()
    output_dir = "./convert_result"
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, os.path.basename(img_name)))



if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='Convert coco format to polygon-yolo')
    parser.add_argument('--input_dir', type=str, default='./coco20', help='COCO data directory')
    parser.add_argument('--save', action='store_true', help='save result images(10)')

    args = parser.parse_args()

    seg2poly(args.input_dir, plot=args.save)
